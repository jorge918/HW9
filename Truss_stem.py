...
class TrussView():
    ...
    def drawAGrid(self, DeltaX=10, DeltaY=10, Height=320, Width=180, CenterX=120, CenterY=60):
        ...
        while y > CenterY - Height / 2:
            self.scene.addLine(CenterX - Width / 2, y, CenterX + Width / 2, y, pen)
            y -= DeltaY

        x = CenterX
        while x > CenterX - Width / 2:
            self.scene.addLine(x, CenterY - Height / 2, x, CenterY + Height / 2, pen)
            x -= DeltaX
        pass
    ...
    def drawLinks(self, truss=None):
        ...
        for link in truss.links:
            ...
            if node1 and node2:
                self.scene.addLine(node1.position.x, node1.position.y, node2.position.x, node2.position.y, pen)
        pass
    ...
    def drawNodes(self, truss=None, scene=None):
        ...
        for node in truss.nodes:
            self.scene.addEllipse(node.position.x - 5, node.position.y - 5, 10, 10, pen, brush)
        pass

    def drawALabel(self, x, y, str='', pen=None, brush=None, tip=None):
        """
           Draws a label at the specified position with the given text.
           """
        # Create a QGraphicsTextItem to display the text
        label = qtw.QGraphicsTextItem(text)
        label.setPos(x, y)

        # Set the pen and brush for the label if provided
        if pen:
            label.setDefaultTextColor(pen.color())
        if brush:
            label.setTextInteractionFlags(qtc.Qt.TextEditorInteraction)
            label.setTextInteractionFlags(qtc.Qt.TextEditorInteraction)
        if tooltip:
    ...
    def drawACircle(self, centerX, centerY, Radius, angle=0, brush=None, pen=None, name=None, tooltip=None):
        """
            Draws a circle (ellipse) at the specified center coordinates with the given radius.
            """
        # Create a QGraphicsEllipseItem to represent the circle
        circle = qtw.QGraphicsEllipseItem(centerX - radius, centerY - radius, 2 * radius, 2 * radius)

        # Set the pen and brush for the circle if provided
        if pen:
            circle.setPen(pen)
        if brush:
            circle.setBrush(brush)
        if name:
            circle.setData(qtc.Qt.UserRole, name)
        if tooltip:
